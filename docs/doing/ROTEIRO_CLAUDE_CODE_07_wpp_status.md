...
## Mapeamento executado - 2026-06-11

1. `wpp_status` ja era atualizado no backend por `WebhooksService` a partir de `ConnectedCallback`, `DisconnectedCallback` e `StatusInstanceCallback` da Z-API, e tambem pelo polling de `/api/whatsapp/status` em `ZapiController`. A implementacao agora centraliza essa atualizacao em `WhatsappStatusService`, mantendo o adapter/provider separado.
2. As notificacoes ja eram disparadas por `NotificacoesService` e chamadas de forma assincrona por `AgendamentosService`. Quando o provider retornava falha/erro durante tentativa de envio, ja havia auditoria parcial; porem, quando o WhatsApp ja estava desconectado, `getWppConfig()` retornava `null` e a notificacao era ignorada sem registro persistente.
3. Nao foi encontrado provedor de e-mail transacional configurado. Foi criada a abstracao `NotificadorEmail` com adapter stub/log e TODO explicito para SMTP/transacional; o badge funciona independentemente disso.
4. Valores de `wpp_status` no codigo: `desconectado`, `conectando`, `conectado`, `erro`. A migracao adiciona check constraint para esses valores.

## Implementado nesta execucao

- `WhatsappStatusService` registra `wpp_desconectado_em` na queda, limpa o timestamp e o flag ao reconectar, e deduplica o aviso por evento com `wpp_aviso_desconexao_em`.
- Aviso por e-mail passa por `NotificadorEmail`; como nao ha provider configurado, o stub loga o TODO e marca a tentativa para evitar spam por evento.
- Dashboard exibe badge acessivel de `WhatsApp desconectado` com acao de reconectar enquanto houver queda registrada.
- Notificacoes bloqueadas por WhatsApp desconectado/sem credenciais sao registradas em `whatsapp_notificacoes` como `nao_enviada`; falhas do provider sao registradas como `falha`; envios ok como `enviada`.
- Fila de reenvio nao foi implementada.
